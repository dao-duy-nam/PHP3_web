        ...
        <div class="card-body">
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="ten_san_pham" value="{{$product->ten_san_pham}}" name="ten_san_pham" class="form-control @error('ten_san_pham') is-invalid @enderror">
                    @error('ten_san_pham')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="ma_san_pham" value="{{$product->ma_san_pham}}" name="ma_san_pham"
                ...
                <div class="mb-3">
                    ...
                    <input type="date" id="ngay_nhap" value="{{$product->ngay_nhap}}" name="ngay_nhap" class="form-control">
                    @error('ngay_nhap')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select id="category_id" name="category_id" class="form-select">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{old('category_id',$product->category_id)==$category->id ?'selectes':''}}>{{ $category->ten_danh_muc }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <p class="text-danger">{{ $message }}</p>
                ...
                <div class="mb-3 row">
                    <div class="col-md-6">
                        ...
                        <input type="number" id="gia" value="{{$product->gia}}" name="gia" class="form-control" min="0">
                        @error('gia')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" value="{{$product->gia_khuyen_mai}}" id="gia_khuyen_mai" name="gia_khuyen_mai" class="form-control" min="0">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" value="{{$product->so_luong}}" id="so_luong" name="so_luong" class="form-control" min="1">
                    @error('so_luong')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <br>
                    @if ($product->hinh_anh)
                    <img src="{{asset('storage/'. $product->hinh_anh)}}" alt="" width="200px">
                    @else 
                    Không có hình ảnh
                    @endif
                </div>
